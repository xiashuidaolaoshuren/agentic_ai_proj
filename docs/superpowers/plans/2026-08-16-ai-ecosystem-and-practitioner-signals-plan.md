...
- [x] **Do:** Add `SourceKind.HUGGINGFACE` and `SourceKind.ZHIHU`, plus a JSON-safe `NewsItem.source_evidence` mapping that defaults to `{}`. Prove model serialization and storage round-trip compatibility for old rows without the field and new rows with populated evidence. No SQLite migration.
