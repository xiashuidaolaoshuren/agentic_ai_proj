...
5. Try `openclaw-followup --message "follow up on item 1"` and expect a persist-only rank deep-dive for that display rank (Juya issue deep-dive, Hugging Face family card, Zhihu practitioner-insight card, or generic digest-item reprint — not a live Hub/Zhihu fetch).
